...
export default function CreateTests() {
    ...
    const fetchTests = async () => {
        setLoading(true);
        try {
            const res = await axios.get("http://localhost:3000/api/tests/mine", { withCredentials: true });
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.post("http://localhost:3000/api/tests", formData, { withCredentials: true });
            setShowModal(false);
            setFormData({
