...
export default function MentorSessions() {
  ...
  const fetchSessions = async () => {
    setLoading(true);
    try {
      const res = await axios.get("http://localhost:3000/api/sessions/mentor", { withCredentials: true });
  ...
  const fetchStudents = async () => {
    try {
      const res = await axios.get("http://localhost:3000/api/mentor/students", { withCredentials: true });
  ...
  const createSession = async () => {
    ...
    setSaving(true);
    try {
      await axios.post("http://localhost:3000/api/sessions", form, { withCredentials: true });
      setShowCreate(false);
  ...
  const updateStatus = async (id, status) => {
    try {
      await axios.put(`http://localhost:3000/api/sessions/${id}/status`, { status }, { withCredentials: true });
  ...
  const saveMeetingLink = async () => {
    ...
    setSaving(true);
    try {
      await axios.put(`http://localhost:3000/api/sessions/${showLinkModal}/meeting-link`, { meetingLink: linkInput.trim() }, { withCredentials: true });
              ...
              <div key={session._id} style={{
                ...
                <div style={{
                  ...
                  {student.profilePicture
                    ? <img src={student.profilePicture.startsWith("http") ? student.profilePicture : `http://localhost:3000/${student.profilePicture.replace(/^\//, "")}`} alt={student.name} style={{ width: "100%", height: "100%", borderRadius: "50%", objectFit: "cover" }} />
                    : initials}
                </div>
