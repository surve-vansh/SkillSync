...
const Notifications = () => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:3000/api/notifications?role=${currentRole}`,
  ...
  const markAllRead = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:3000/api/notifications/read-all?role=${currentRole}`,
        {},
        { withCredentials: true }
      );
  ...
  const handleAction = async (n) => {
    try {
      await axios.put(
        `http://localhost:3000/api/notifications/${n._id}/read`,
        {},
        { withCredentials: true }
      );
